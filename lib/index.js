...
class Client {
    ...
    try {
      ...
      if (!this.subToken || !db) {
        ...
        props = {
          baseURL: this.options.url,
          headers: {
            Authorization: 'Bearer ' + r.data.results.token,
          }
        }
        if (typeof window === 'undefined') {
          props.headers['User-Agent'] = 'Unbounded JS Client';
        }
        this.dbClients[db] = axios.create(props);
      }
    } catch (e) {
      if (cb) {
        cb(getErr(e));
  ...
  async listDatabases(cb) {
    try {
      let r = await this.request(null, {method: 'get', url: '/databases'});
      if (cb) {
        cb(null, r);
      } else {
        return r;
...
class Database extends QueryBase {
  ...
  async getKey(cb) {
    try {
      let r = (await this.client.request(this.db, {method: 'get', url: this.url})).results.key || null;
      if (cb) {
        cb(null, r);
      } else {
        return r;
  ...
  async getSchema(cb) {
    try {
      let r = (await this.client.request(this.db, {method: 'get', url: this.url})).results.schema || null;
      if (cb) {
        cb(null, r);
      } else {
        return r;
  ...
  async getIndexes(cb) {
    try {
      let r = (await this.client.request(this.db, {method: 'get', url: this.url})).results.indexes || [];
      if (cb) {
        cb(null, r);
      } else {
        return r;
  ...
  async listSavedQueries(cb) {
    try {
      let r = await this.client.request(this.db, {method: 'get', url: this.url + '/savedqueries'});
      if (cb) {
        cb(null, r);
      } else {
        return r;
